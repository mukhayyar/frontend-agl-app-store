  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <nav className="mb-4" aria-label="Breadcrumb">
        <Link href="/" className="text-primary underline underline-offset-4" aria-label="Go to Home">
          {"← Home"}
        </Link>
      </nav>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {brands.map((b) => (
          <Link key={b.slug} href={`/brands/${b.slug}`} className="no-underline">
            <Card className="bg-card hover:bg-accent transition-colors">
              <CardHeader className="flex flex-row items-center gap-4">
                <div className="h-12 w-12 rounded-md overflow-hidden bg-muted flex items-center justify-center">
                  <Image src={b.logoUrl || "/placeholder-logo.svg"} alt={`${b.name} logo`} width={48} height={48} />
                </div>
